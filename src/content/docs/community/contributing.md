...
- **Expert review** — new works, new citation systems, new corpora, contested mappings, changes to deterministic ID inputs, status changes (`verified` / `disputed` / `withdrawn` / `merged` / `blocked`). Needs technical validation, a documented rationale with sources, and at least one expert reviewer.
...
1. Branch from `dev` (this is the integration branch; `main` is the release branch).
2. Keep PRs focused — one logical change per PR.
3. Link related issues in the PR description.
4. Make sure `npm run verify` passes.
5. Open the PR against `dev`. The maintainers will assign reviewers based on the track.
